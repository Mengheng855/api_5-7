                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @auth
                            <form action="{{url('/auth/logout')}}" method="post">
                                @csrf
                                <button class="bg-slate-900 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-slate-800 transition-colors">Logout</button>
                            </form>
                        @endauth
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                ...
                <div class="bg-white rounded-2xl overflow-hidden border border-slate-200 hover-lift">
                    <div class="aspect-square bg-slate-100 flex items-center justify-center overflow-hidden">
                        <img src="{{ $pro['image'] }}" alt="{{ $pro['name'] }}" class="w-full h-full object-cover">
                    </div>
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-2">
                            <h3 class="text-xl font-semibold text-slate-900">{{ $pro['title'] }}</h3>
                            <span class="text-lg font-bold text-slate-900">${{ $pro['price'] }}</span>
                        </div>
                        <p class="text-sm text-slate-500 mb-4">{{ $pro['sub_title'] }}</p>
                        <p class="text-slate-600 text-sm mb-4 leading-relaxed">{{ $pro['description'] }}</p>
